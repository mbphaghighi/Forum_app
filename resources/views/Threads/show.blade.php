            ...
            <div class="col-md-8">
                <div class="card">
                    <div class="card-header"><h3 style="float: left"><a
                                    href="{{route('base')}}{{ $thread->path() }}">{{ $thread->title }}</a></h3>
                        ...
                        <div class="card" id="reply-{{$reply->id}}">
                            <div class="card-header">
                                <h5 style="display: flex; flex: 1; float: left">
                                    <a href="{{ route('profile', $reply->owner->name) }}" style="margin-right: 8px">
                                        {{ $reply->owner->name }}
                ...
                @if(auth()->check())
                    <form action="{{route('base')}}{{ $thread->path() }}/addReply" method="POST">
                        @csrf
                        <div class="form-group">
                ...
                <div class="card card-default">
                    <div class="card-body">
                        ...
                        @if(!$thread->isSubscribed)
                            <p>
                            <form action="{{route('base')}}{{ $thread->path() }}/subscription" method="POST">
                                @csrf
